...
if ($_POST) {
  ...
  foreach ($dato as $d) {
      ...
      if ($d['symbol']=='XRP')      {$quote2=$d['quote'];}
      if ($d['symbol']=='XLM')      {$quote3=$d['quote'];}
      if ($d['symbol']=='FARTCOIN') {$quote4=$d['quote'];}
      if ($d['symbol']=='WLD')      {$quote5=$d['quote'];}
    } 
  foreach ($quote as $d) {
      $price=$d['price'];
...
echo json_encode($price.'@'.$price2.'@'.$price3.'@'.$price4.'@'.$price5);
